      ...
      <div className="bg-[#0f172a] border-4 border-black shadow-[8px_8px_0px_#000000] max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 md:p-6 bg-[#0a1622] overflow-y-auto space-y-4 text-slate-100 flex-1">
          ...
          <div className="bg-[#111f30] p-3 border-2 border-black space-y-2">
            ...
            <ul className="grid grid-cols-1 gap-1.5 font-vt323 text-base text-slate-200">
              {project.highlights.map((hl, idx) => (
                <li key={idx} className="flex items-center gap-2">
                  <span className="text-yellow-400 font-pixel text-[8px]">▶</span>
                  <span>{hl}</span>
                </li>
              ))}
            </ul>
          ...
          {/* Tools & Inventory */}
          <div className="bg-[#111f30] p-3 border-2 border-black space-y-2">
            <h4 className="font-pixel text-[9px] text-cyan-400 flex items-center gap-1.5">
              <Wrench className="w-3 h-3" />
              <span>TOOLS & GEAR YANG DIGUNAKAN:</span>
            </h4>
            <div className="flex flex-wrap gap-1.5">
              {project.tools.map((tool, idx) => (
                <span
                  key={idx}
                  className="bg-[#162a42] text-yellow-300 font-pixel text-[8px] px-2 py-1 border border-black shadow-[2px_2px_0px_#000]"
                >
                  {tool}
                </span>
              ))}
        ...
        {/* Footer Actions */}
        <div className="bg-[#1e293b] p-3 border-t-4 border-black flex items-center justify-end">
          <button
            onClick={() => {
              soundManager.playWindowClose();
              onClose();
